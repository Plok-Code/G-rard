...
function buildActorProfile(actor, options = {}) {
  ...
  const mention =
    mentionActor && match.discordUserId ? `<@${match.discordUserId}>` : `**${displayName}**`;
  return { mention, label: displayName };
}

function buildPrefix(actorMention) {
  return [actorMention].filter(Boolean).join(' ').trim();
}

function withRoleSuffix(content, roleMention) {
  return [content.trim(), roleMention].filter(Boolean).join(' ').trim();
}
...
function formatPullRequest(event, payload, roleMention, actor) {
  ...
  const prefix = buildPrefix(actor.mention);
  const content = withRoleSuffix(`${prefix} ${actionText} la PR ${prLink}`, roleMention);
...
function formatPush(event, payload, roleMention, actor) {
  ...
  const prefix = buildPrefix(actor.mention);
  const content = withRoleSuffix(
    `${prefix} a pousse ${commits.length} commit(s) sur \`${branch}\` (${repoName}).`,
    roleMention,
  );
...
function formatIssueComment(event, payload, roleMention, actor) {
  ...
  const content = withRoleSuffix(`${prefix} a commente ${targetLabel} ${issueLink}.`, roleMention);
...
function formatCommitComment(event, payload, roleMention, actor) {
  ...
  const prefix = buildPrefix(actor.mention);
  const content = withRoleSuffix(
    `${prefix} a commente le commit \`${commitId}\` dans ${payload.repository?.full_name || 'Repository'}.`,
    roleMention,
  );
  const embed = {
    color: 0x24292e,
...
function formatPullRequestReview(event, payload, roleMention, actor) {
  ...
  const prefix = buildPrefix(actor.mention);
  const prLink = `[${pr.title} (#${pr.number})](${pr.html_url})`;
  const content = withRoleSuffix(`${prefix} ${verb} ${prLink}.`, roleMention);
...
function formatDelete(event, payload, roleMention, actor) {
  ...
  const prefix = buildPrefix(actor.mention);
  const content = withRoleSuffix(`${prefix} a supprime ${refType} \`${ref}\` dans ${repo}.`, roleMention);
...
function formatGithubEvent(eventName, payload, roleMention, options = {}) {
  const actor =
    payload.sender ||
    payload.pusher ||
    ...
    null;
  const actorProfile = buildActorProfile(actor, options);
